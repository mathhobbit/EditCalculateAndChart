...
//The code was modified by S. Nikitin on 11/30/2016 in order to accomodate
//rela polinomials with only complex coefficients. It twice speeds up the root 
//calculation. The real roots for polynomials with real coefficients could be
// efficiently calculated with "Leap Gradient Algorithm" proposed by S. Nikitin
...
public static Complex[] findRoots (double[] coeffs){
    return findRoots (coeffs,false);
}
public static Complex[] findRoots (double[] coeffs,
                                   boolean Real_Polynomial_With_Only_Complex_Roots) {

    
    return findRoots(ArrayUtils.toComplex(coeffs),Real_Polynomial_With_Only_Complex_Roots); 
}
...
public static Complex[] findRoots (Complex[] coeffs,
   ...
   for (int i = 0; i < n; i++) {
      ...
      if(Real_Polynomial_With_Only_Complex_Roots && zero!=null){
          a = PolynomialUtils.deflate(a, zero.conj(), 0);
      }
   }
