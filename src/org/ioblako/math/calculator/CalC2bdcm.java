...
public class CalC2bdcm implements CalcFunction {
String Report="";
public String getHelp(){
return "convert each number in your formula into BigDecimal form.";
}
...
for(String bb:els){
  if(!bb.contentEquals("")){
       ...
       rt=String.format("%s%s%s",rt,tmp.substring(0,tmp.indexOf(bb)),
                       (new BigDecimal(bb,jc.MC)).toPlainString());
       else
       rt=String.format("%s%s%s",rt,tmp.substring(0,tmp.indexOf(bb)),
                      jc.getFraction(bb).toBigDecimal(jc.MC).toPlainString());
   }
