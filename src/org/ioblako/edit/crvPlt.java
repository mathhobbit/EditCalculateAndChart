 ...
 */
public class crvPlt extends AbstractFramePlt implements FramePlt {
...
ChartPanel chartP=null;
    ...
    public crvPlt(String txt) throws Exception{
        super("");
        toShow=txt.trim();


      preProcess("crvPlt(",toShow);
      FunctionToPlot =toShow.substring(toShow.indexOf("crvPlt(")+7);
       if(!FunctionToPlot.startsWith("{"))
               throw new Exception("{ and } are missing !!!");
      FunctionToPlot = FunctionToPlot.substring(1, FunctionToPlot.indexOf("}"));

 
           function = new ArrayList<String>();
  
      for(String bf:FunctionToPlot.split(";"))
            function.add(bf);       
...
BigDecimal pt, rightEnd=rangesRight.get(0), step=steps.get(0), Arg, Val;
String bf, strArg, strVal;
...
while(pt.compareTo(rightEnd)<0){
   bf=SmartReplace.get(newFunction[i],Var,pt.toPlainString());
strArg = jc.eval(bf.substring(0,bf.indexOf(",")));
strVal = jc.eval(bf.substring(bf.indexOf(",")+1));
if(strArg.contains("/"))
    Arg = (new Fraction(strArg)).toBigDecimal();
else
    Arg = new BigDecimal(strArg);

if(strVal.contains("/"))
    Val = (new Fraction(strVal)).toBigDecimal();
else
    Val = new BigDecimal(strVal);


 series[i].add(Arg,Val);
// series[i].add(new BigDecimal(jc.eval(bf.substring(0,bf.indexOf(",")))),
//               new BigDecimal(jc.eval(bf.substring(bf.indexOf(",")+1))));
   pt=pt.add(step);
}
i++;
}
...
//this.setVisible(true);
    ...
    public String getHelp() {
        return "Curve plot is created with" +System.lineSeparator()+
"" +System.lineSeparator()+
"crvPlt({a,a*a},a={0.1,0..2}) " +System.lineSeparator()+
"" +System.lineSeparator()+
"crvPlt({a,a*a;a^2-1,a^3 - a},a={0.1,0..2})" +System.lineSeparator()+
"" +System.lineSeparator()+
